...
func (p *AttributeNotEqualsPredicate) EvaluateNode(node *gsl.Node) bool {
	...
	// Special case: "id" refers to the node's ID
	if p.Name == "id" {
		return node.ID != p.Value
	}
	if node.Attributes == nil {
		return false // Missing attribute evaluates false
	}
	val, exists := node.Attributes[p.Name]
	if !exists {
		return false // Missing attribute evaluates false
...
func (p *AttributeNotEqualsPredicate) TargetType() string { return p.Target }
...
// ParentNotExistsPredicate matches edges with no parent (top-level)
type ParentNotExistsPredicate struct {
	Target string // "node" or "edge"
}
...
type DepthPredicate struct {
	Target   string // "node" or "edge"
	Operator string // "==" or "!="
	Value    int    // expected depth
}

func (p *DepthPredicate) EvaluateNode(node *gsl.Node) bool { return false }

func (p *DepthPredicate) EvaluateEdge(edge *gsl.Edge) bool {
	if edge == nil {
		return false
	}
	depth := p.computeDepth(edge)
	switch p.Operator {
	case "==":
		return depth == p.Value
	case "!=":
		return depth != p.Value
	default:
		return false
...
func parseNodePredicate(input string) (Predicate, error) {
	...
	// Find the !=, ==, or = sign (check != first as it's longer)
