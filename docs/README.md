...
The agent-oriented index and embedded guides sit at the repository root: start at [llms.txt](../llms.txt), then [GSL_GUIDE](../GSL_GUIDE.md), [GQL_GUIDE](../GQL_GUIDE.md) and [GO_REFERENCE](../GO_REFERENCE.md). The [LLM-assisted modelling flagship](../examples/flagships/05-llm-assisted-modelling/README.md) is the worked experiment. The [architecture archaeology study](../examples/advanced/architecture-archaeology/README.md) shows how that approach scales from one system to many repositories. The human learning path above does not require any LLM mental model.